...
if __name__ == '__main__':
    ...
    device = torch.device(str_cuda_device if (
    ...
    trajectory_prediction_datasets = process_data_directory_surgery(
        ...
        skip_nan = True,
        seed=1234, phase_translation_file=args.phase_translation_file, cache_dir=args.cache_dir, params=vars(args))
    ...
    trainer = TemporalTrainer(temporal_model,
                                 params=params,
                                 data = dataloader,
                                 device=device,
                                 logger=logger)
    ...
    if os.path.exists(fullmodel_filename) and args.load_pretrained_model:
        ...
        for it in tqdm.tqdm(range(args.phase_pretrain_iter),desc='phase identification iterations'):
            ...
            if it == args.phase_pretrain_iter / 2:
                ...
                trainer.phase_identification_optimizer = torch.optim.Adam(params=trainer.phase_id_params,
                                                                          lr=0.1 * trainer.learning_rate,
                                                                          weight_decay=1e-4)
