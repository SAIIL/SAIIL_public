...
python $sdtm_home/scripts/train_temporal_model.py  --data_dir ${data_dir} --annotation_filename ${annotation_dir} --log_dir $sdtm_home/logs_gan --learning_rate 4e-5 --mse_coeff 0.2 --training_ratio 1.0  --sampling_rate $sampling_rate --model_filename $sdtm_home/temporal_weights/${script_arguments}_statedict.pt --phase_order_filename $sdtm_home/mgh_phase_order.json --phase_length_filename $sdtm_home/LapChole_phase_length_mgh100.json --cache_dir $cache_dir --num_dataloader_workers ${num_dataloader_workers_trainer} --batch_size 8 --track_name phase
