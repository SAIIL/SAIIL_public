...
class TemporalTrainer():
    def __init__(self, model, data, params, device, logger):
        ...
        self.phase_identification_optimizer = torch.optim.Adam(params=self.phase_id_params,
        ...
        self.data_loss_past = torch.nn.CrossEntropyLoss(weight=self.real_dataset.dataset.class_weights.float().to(device))
    ...
    def reshuffle_data(self, epoch_size = 128):
        indices = np.random.choice(a = len(self.real_dataset.dataset),
                                   size = epoch_size,
                                   p = self.real_dataset.dataset.segment_weights_list)
    ...
    def train_phase_identification(self, pretrain_flag = 'visual'):
        ...
        if self.logger:
            ...
            self.logger.log_train(self.total_batch_cnt,
                                  scalars,
                                  None,
    ...
    def phase_identification_cost(self, inps, imgs, pretrain_flag = 'lstm'):
        ...
        for t in range(past_sample_prob.shape[1]):
            cost += self.data_loss_past(
                past_sample_prob[:, t, :],
                labels[:, t].squeeze().clone().detach().long())
