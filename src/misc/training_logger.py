...
class TrainingLogger:
    ...
    def log_train(self, step, scalars, weights, prefix=None,text={},fig={}):
        ...
        if weights:
            for x in weights.keys():
                self.train_logger.add_histogram(tag=str_prefix+'train/'+str(x),bins=weights[x]['bins'],values=weights[x]['values'],global_step=step)
        for x in fig.keys():
    ...
    def log_test(self, step, scalars, weights, prefix=None,text={},fig={}):
        ...
        if weights:
            for x in weights.keys():
                self.test_logger.add_histogram(tag=str_prefix+'test/'+str(x),bins=weights[x]['bins'],values=weights[x]['values'],global_step=step)
        for x in fig.keys():
